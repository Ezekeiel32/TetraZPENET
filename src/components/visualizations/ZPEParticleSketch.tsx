    ...
    if (typeof window !== 'undefined') {
      import('p5').then(p5Module => {
        ...
        const sketch = (p: p5) => {
          ...
          const TYPES = {
              QUARK: { size: 4, speed: 1.5, color: [0, 90, 95], memory: 2 },
          ...
          class Particle {
              ...
              constructor(type?: string) {
                  ...
                  this.baseColor = [...this.typeProps.color];
                  this.scanned = false;
                  this.inMemory = false;
              ...
              update() {
                  ...
                  if (this.scanned && p.frameCount % 60 === 0) {
                      this.scanned = false;
                  }
              }
          ...
          class TetrahedralNode {
              ...
              update() {
                  ...
                  if (this.active) {
                      ...
                      if (this.storedParticle) {
                          this.processingPower = this.storedParticle.dataValue / 100;
                      }
                  } else {
                      this.pulseSize = p.max(0, this.pulseSize - 0.2);
                      this.processingPower *= 0.95;
                  }
              }
              ...
              display() {
                  ...
                  if (this.active) {
                      p.fill(60, 100, 100, 70); p.stroke(60, 100, 100);
                  } else {
                      p.fill(210, 70, 50, 50); p.stroke(210, 70, 80);
                  }
          ...
          function initTetrahedralMemory() {
              ...
              tetrahedralMemory.nodes.push(new TetrahedralNode(P5.Vector.lerp(center, v1, 0.3), 12));
              createTetrahedralConnections();
          }
          
          function createTetrahedralConnections() {
              addMemoryConnection(0, 1); addMemoryConnection(1, 2); addMemoryConnection(2, 0);
              addMemoryConnection(0, 3); addMemoryConnection(1, 3); addMemoryConnection(2, 3);
              ...
              if (p.frameCount % 30 === 0) {
                  ...
                  if (leptons.length >= 2) {
                      for (let i = 0; i < p.min(5, leptons.length); i++) {
                          ...
                          let lepton2 = findClosestParticles(lepton1, leptons, 1)[0];
                          if (lepton1 && lepton2 && lepton1 !== lepton2) {
          ...
          function updateScanner() {
              if (!scanner.active) {
                  if (p.random() < 0.005) {
                      scanner.active = true;
                      scanner.position = P5.Vector.random3D().mult(100);
              ...
              if (scanner.position) {
                ...
                scanner.position.y = p.sin(scanner.angle * 1.5) * 60;
              }
              ...
              if (p.random() < 0.01) scanner.active = false;
          }
          ...
          function renderScanner() {
              ...
              p.noFill(); p.stroke(120, 100, 100, 20); p.strokeWeight(0.5); p.sphere(scanner.radius);
              p.fill(120, 100, 100, 50); p.noStroke(); p.sphere(5);
          ...
          function updateMemoryProcessingPower() {
              ...
              totalPower *= (1 + activeConnections.length / 20);
          ...
          function renderMemoryConnections() {
              for (let conn of tetrahedralMemory.connections) {
                  ...
                  if (conn.active) {
                      let pulse = (p.sin(p.frameCount * 0.2 + conn.pulsePhase) + 1) / 2;
                      p.stroke(60, 100, 100, 30 + conn.strength * 70 * pulse);
                      p.strokeWeight(0.5 + conn.strength * pulse);
                  } else {
                      p.stroke(210, 30, 50, 20); p.strokeWeight(0.3);
                  }
          ...
          p.draw = () => {
              ...
              updateScanner();
              if (p.frameCount % 10 === 0) connectParticles();
              for (let conn of tetrahedralMemory.connections) {
                  if (conn.active) {
                      conn.strength *= 0.99;
