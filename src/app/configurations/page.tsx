...
export default function ConfigurationsPage() {
  ...
  const fetchJobsList = useCallback(async () => {
    ...
    try {
      ...
      setJobsList((data.jobs || []).sort((a,b) => new Date(b.start_time || 0).getTime() - new Date(a.start_time || 0).getTime()));
    } catch (error) {
      ...
      {selectedJobDetails && (
          ...
          <DialogContent className="max-w-2xl">
            ...
            <ScrollArea className="max-h-[60vh] p-1 pr-3">
              <div className="space-y-3 text-sm py-4">
                ...
                <p><strong>Cycle Length:</strong> {selectedJobDetails.parameters.cycleLength}</p>
                ...
                {selectedJobDetails.baseConfigId && <p><strong>Base Config ID:</strong> {selectedJobDetails.baseConfigId}</p>}
                 <div className="mt-4 pt-4 border-t">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Note: Retrain button would pre-fill the training form. (Functionality to be fully implemented)</p>
                </div>
              </div>
            </ScrollArea>
            <DialogFooter>
       ...
       <Card className="mt-6 bg-accent/30 border-accent">
        ...
        <CardContent className="text-sm text-accent-foreground/80 space-y-2">
            ...
            <p>The parameters displayed are those submitted when the job was started and are simulated by the backend training process.</p>
        </CardContent>
      </Card>
    </div>
  );
