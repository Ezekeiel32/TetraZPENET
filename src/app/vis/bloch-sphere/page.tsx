...
export default function BlochSpherePage() {
  const [theta, setTheta] = useState(Math.PI / 2); // Initial polar angle (equator)
  const [phi, setPhi] = useState(0);               // Initial azimuthal angle (positive x-axis)
  ...
  const handleThetaChange = (value: number[]) => {
    setTheta(value[0]);
  };

  const handlePhiChange = (value: number[]) => {
    setPhi(value[0]);
    // If not evolving, directly update the sketch's base phi
    // The sketch itself will also listen to prop changes
  };
        ...
        <div className="md:col-span-1 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button onClick={toggleEvolution} className="w-full">
